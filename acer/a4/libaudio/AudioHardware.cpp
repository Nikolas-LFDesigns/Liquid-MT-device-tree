...
static uint32_t SND_DEVICE_HEADSET_NO_MIC=2;
...
static uint32_t DEVICE_handset_dual_mic_endfire_tx = 1;//handset_dual_mic_endfire_tx
static uint32_t DEVICE_SPEAKER_RX = 2; //speaker_stereo_rx
static uint32_t DEVICE_SPEAKER_TX = 3;//speaker_mono_tx
static uint32_t DEVICE_headset_stereo_rx = 4; //headset_stereo_rx
static uint32_t DEVICE_headset_mono_tx = 5; //headset_mono_tx
...
static uint32_t DEVICE_DUALMIC_handset_dual_mic_endfire_tx = 9; //handset_dual_mic_endfire_tx
...
static uint32_t DEVICE_SPEAKER_headset_stereo_rx = 13; //headset_stereo_speaker_stereo_rx
  ...
  //          LOGE("msm_reset_all_device() failed");
        ...
        for(i = 0; i < dev_cnt;i++) {
            if(strcmp((char* )name[i],"handset_rx") == 0)
                ...
                index = DEVICE_handset_dual_mic_endfire_tx;
                ...
                index = DEVICE_headset_stereo_rx;
            else if(strcmp((char* )name[i],"headset_mono_tx") == 0)
                index = DEVICE_headset_mono_tx;
                ...
                index = DEVICE_DUALMIC_handset_dual_mic_endfire_tx;
                ...
                index = DEVICE_SPEAKER_headset_stereo_rx;
                                   ...
                                   bool ear_mute, bool mic_mute)
{
    ...
    if(device == SND_DEVICE_HANDSET) {
        ...
        new_rx_device = DEVICE_SPEAKER_headset_stereo_rx;
        new_tx_device = DEVICE_handset_dual_mic_endfire_tx;
        LOGV("In DEVICE_SPEAKER_headset_stereo_rx and DEVICE_handset_dual_mic_endfire_tx");
    }
    ...
    if (mState < AUDIO_INPUT_OPENED) {
        ...
        if((mDevices == AudioSystem::DEVICE_IN_FM_RX) || (mDevices == AudioSystem::DEVICE_IN_FM_RX_A2DP) ){
            ...
            if(cur_tx == INVALID_DEVICE)
                cur_tx = DEVICE_handset_dual_mic_endfire_tx;
